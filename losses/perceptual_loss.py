...
class PerceptualLoss(nn.Module):
    def __init__(self, feature_layer=[2,7,16,25,34], weights=[0.1,0.1,1.0,1.0,1.0], lossfn_type='mse', use_input_norm=True, use_range_norm=False):
        '''
           VGG Perceptual loss
        :param feature_layer: 如果输入list 则为特征层加权 loss 求和，否则就是单个特征层
        :param weights: 加权值
        :param lossfn_type: 损失函数类型 mse or l1
        :param use_input_norm: use_input_norm: If True, x: [0, 1] --> (x - mean) / std
        :param use_range_norm: use_range_norm: If True, x: [0, 1] --> x: [-1, 1]
        '''
